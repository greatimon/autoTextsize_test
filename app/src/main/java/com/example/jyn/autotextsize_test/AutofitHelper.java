...
public class AutofitHelper {
    ...
    private static float mPrecision;
    ...
    private static void autofit(TextView view, TextPaint paint, float minTextSize, float maxTextSize,
        ...
        if (targetWidth <= 0 || (maxLines <= 0 || maxLines == Integer.MAX_VALUE)) {
            return;
        }
        ...
        if ((maxLines == 1 && paint.measureText(text, 0, text.length()) > targetWidth)
            ...
            size = getAutofitTextSize(text, paint, targetWidth, maxLines, low, high, displayMetrics);
        }
        setSizeToView(view, size, minTextSize);

    }

    private static void setSizeToView(TextView view, float size, float mTextSize) {
        float mSize = size;
        if (mSize < mTextSize) {
            mSize = mTextSize;
        }
    ...
    private static float getAutofitTextSize(CharSequence text, TextPaint paint,
        ...
        if (lineCount > maxLines) {
            ...
            data.put("low", low);
            data.put("high", high);
            data.put("displayMetrics", displayMetrics);
            data.put("targetWidth", targetWidth);
    ...
    private static float size(Map data) {
        float maxLineWidth = 0;
        int maxlines = (int) data.get("maxLines");
        TextPaint paint = (TextPaint) data.get("paint");
        CharSequence text = (CharSequence) data.get("text");
        StaticLayout layout = (StaticLayout) data.get("layout");
        int lineCount = (int) data.get("lineCount");
        float low = (float) data.get("low");
        float high = (float) data.get("high");
        DisplayMetrics displayMetrics = (DisplayMetrics) data.get("displayMetrics");
        float targetWidth = (float) data.get("targetWidth");

        maxLineWidth = getMaxWidth(maxlines, maxLineWidth, paint, text, layout, lineCount);
    ...
    private static float getMaxWidth(int maxLines, float maxLineWidth, TextPaint paint, CharSequence text, StaticLayout layout, int lineCount) {
        float width = maxLineWidth;
        if (maxLines == 1) {
            width = paint.measureText(text, 0, text.length());
        } else {
            width = calMaxWidth(lineCount, width, layout);
        }
        return width;
    }
     ...
     */
    private static float calMaxWidth(int lineCount, float maxLineWidth, StaticLayout layout) {
        float calWidth = 0;
    ...
    public AutofitHelper setPrecision(float precision) {
        if (Float.compare(mPrecision, precision) != 0) {
    ...
    private void setRawMinTextSize(float size) {
        if (Float.compare(size, mMinTextSize) != 0) {
    ...
    private void setRawMaxTextSize(float size) {
        ...
        if (Float.compare(size, mMaxTextSize) != 0) {
            mMaxTextSize = size;
    ...
    private void setRawTextSize(float size) {
        ...
        if (Float.compare(size, mTextSize) != 0) {
            mTextSize = size;
        }
    }
    ...
    private void autofit() {
        ...
        if (Float.compare(textSize, oldTextSize) != 0) {
